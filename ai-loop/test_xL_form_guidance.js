...
(async () => {
  ...
  console.log('\n[4] 🔴 2단계 제출 — 브라우저가 막는 자리도 우리가 설명한다');
  {
    ...
    /* 🔴 2026-09-14 대표 지시로 2단계에 **연락처 한 칸만** 남았다.
       그래서 「N칸이 남았습니다」가 아니라 **그 칸 이름을 바로 말한다.**
       ⚠ 지키려던 것은 「몇 개가 남았는지 세어 준다」가 아니라 **「무엇을 더 하면
         되는지 알려 준다」**였다. 한 칸일 때는 이름을 말하는 쪽이 더 낫다.
       ⚠ 칸이 다시 늘어나면 「N칸」 문구가 돌아와야 한다 — 그때는 아래를 되돌린다. */
    ok('④ 한 칸만 남았으면 그 칸 이름을 말한다',
      /연락처/.test(msg(2)) && !/[0-9]칸이 남았습니다/.test(msg(2)), msg(2));

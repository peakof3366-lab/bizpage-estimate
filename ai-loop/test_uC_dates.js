...
/* ══ [6] 자간이 벌어진 금액 · 「당」 없는 1인 표기 (UF) ═══════════════════════
   PDF에서 글자가 하나씩 떨어져 나오는 양식이 있다(하나투어 상해):
     「총 합계 비용 **15, 861 , 6 00** 원」  ← 15,861,600원
     「**1 인 1,030,000 원** + 황포강유람선 / 꽃 비용 411,600 원」
   15명 x 1,030,000 + 411,600 = 15,861,600으로 총계와 정확히 맞는데 둘 다 못 읽었다. */
console.log('\n[6] 자간이 벌어진 금액과 「당」 없는 1인 표기');
{
  ln = 0;
  const r = X.readOneBlock([
    at(['인 원', '15']),
    at(['총 합계 비용 15, 861 , 6 00 원']),
    at(['1 인 1,030,000 원 + 황포강유람선 / 꽃 비용 411,600 원']),
  ], {}, null);
  ok('자간이 벌어진 총계를 되살린다', r.itemsTotal === 15861600, String(r.itemsTotal));
  ok('「당」 없는 「1 인 1,030,000」을 1인당으로 읽는다', r.perPerson === 1030000, String(r.perPerson));
}
{
  /* ⚠ 🔴 **실제로 회귀를 냈던 줄이다.** 「1 인」 뒤 숫자 한 자리만 보면 「1인 1실」이 걸려
     그 줄의 큰 금액이 1인당 후보가 되고, 검산에 걸려 **맞는 값까지 함께 버려진다.** */
  ln = 0;
  const r = X.readOneBlock([
    at(['인 원', '24']),
    at(['총견적가', '27,306,720']),
    at(['1인 금액 1,137,780']),
    at(['호텔', '아시아호텔', '90,000', '2', '24명', '4,320,000', '1인 1실']),
  ], {}, null);
  ok('**「1인 1실」을 1인당으로 읽지 않는다**', r.perPerson === 1137780, String(r.perPerson));
}
{
  /* ⚠ 되살리기가 **「환율을 모르면 환산하지 않는다」를 뚫으면 안 된다**(SH·SF).
     회귀 테스트(test_sH)가 실제로 이걸 잡았다 — 「합계 ¥ 2,557,000」을 원화로 읽었다. */
  ln = 0;
  const r = X.readOneBlock([
    at(['인 원', '20']),
    at(['합계 ¥ 2,557,000']),
  ], {}, null);
  ok('외화 줄에는 자간 되살리기를 쓰지 않는다', !r.itemsTotal, String(r.itemsTotal));
}
{
  /* ⚠ 붙였을 때 **세 자리 콤마 형식이 완성될 때만** 금액으로 본다 —
     「인원 15 20」이 1520이 되면 없던 금액이 생긴다. */
  ln = 0;
  const r = X.readOneBlock([
    at(['인 원', '20']),
    at(['합계 인원 15 20 명']),
  ], {}, null);
  ok('콤마 형식이 안 되면 금액으로 안 본다 (없던 금액을 만들지 않는다)',
    !r.itemsTotal, String(r.itemsTotal));
}

/* ═══════════════════════════════════════════════════════════════════════════
   ...
   🔴 잠그는 것 넷:
     ...
     ② 성공하면 **성공했다는 것**이 보인다 — 성공과 실패가 구별돼야 한다
        ⚠ 2026-09-15: 예전엔 「링크가 준비」라는 **글자**를 찾았는데, 그 문구가 담당자
          말투라 고객 문서에서 바뀌자 이 검사가 깨졌다. **글자가 아니라 뜻**을 재도록
          옮겼고(「준비되었습니다」), 고객 경로에서 담당자 말투가 안 나오는지도 함께 본다.
...
(async () => {
  ...
  /* 🔴 **글자가 아니라 뜻을 잰다** (2026-09-15에 옮겼다).
     예전에는 「링크가 준비」라는 **글자**를 찾았는데, 그 문구가 담당자 말투라
     고객 문서에서 바뀌자 이 검사가 깨졌다. 지키려던 것은 「성공했다는 것이
     화면에 보인다」이지 특정 낱말이 아니다 — 재는 자리를 뜻으로 옮긴다.
     ⚠ 이 하네스는 **고객 경로**로 연다(`__INTERNAL_TOOL__`이 없다). */
  ok('보이는 글에 「준비되었습니다」가 있다', /준비되었습니다/.test(성공.보임), 성공.보임.slice(0, 100));
  ok('실패 문구는 없다', !/잠시 문제가 있었습니다/.test(성공.보임));
  /* 🔴 2026-09-15 대표 지적 — **고객이 자기 견적서를 열었는데 담당자용 안내가 떴다**:
       「고객에게 보낼 링크가 준비되었습니다 — …에서 복사하실 수 있습니다」
     고객에게는 자기더러 자기에게 보내라는 말이 된다. 문구를 `__INTERNAL_TOOL__`로
     갈랐고(`test_zH_doc_voice`가 분기 자체를 잠근다), 여기서는 **고객 경로에서 실제로
     담당자 말투가 안 나오는지**를 그려진 글자로 확인한다. */
  ok('② 🔴 고객 경로인데 담당자 말투가 아니다', !/고객에게 보낼/.test(성공.보임),
    성공.보임.slice(0, 120));
  ok('② 고객이 할 수 있는 일을 말한다(인쇄·저장)', /인쇄|저장/.test(성공.보임));

...
console.log('\n[3] 회사/기관명이 더는 길을 막지 않는다');
{
  ...
  /* 🔴 2026-09-14 대표 지시로 라벨이 「회사 / 모임 이름」 → **「회사명」**이 됐다.
     VX에서 「모임 이름」을 라벨에 넣었던 이유는 **가족·친목 손님이 「회사명」만 보고
     자기 자리가 아니라고 읽는 것**을 막기 위해서였다. 그 일은 이제 **예시 문구**가 한다:
       예: ○○주식회사 · 김○○ 가족 · 산악회
     ⚠ 예시는 값을 넣으면 사라진다 — 라벨만큼 강하지 않다. 이 칸에서 이탈이 늘면
       라벨을 「회사 / 모임 이름」으로 되돌리는 것을 먼저 검토한다.
     ⚠ 그리고 **필수가 아니라는 것**이 진짜 방어선이다(바로 위 단언이 그것을 잡는다). */
  ok('③ 모임 손님도 적을 것이 있다고 알린다(예시 문구)',
    /가족/.test(FORM) && /산악회/.test(FORM));

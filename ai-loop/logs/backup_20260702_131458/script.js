...
function getBreakdownData() {
  ...
  /* 조정된 단가 계산 */
  const airUnit   = Math.round(dest.airfare        * paxTier.factor * seasonInfo.factor);
  const fuelUnit  = Math.round(dest.fuel_surcharge * paxTier.factor * seasonInfo.factor);
...
(function attachLiveListeners() {
  ['destination','programType','participants','days','organizationType'].forEach(id => {
...
form.addEventListener('submit', (event) => {
  ...
  (function saveFullEstimate() {
    ...
    const estRecord = {
      ...
      rooms:        Math.ceil(bd.participants / 2),
    ...
    if (typeof _trackEvent !== 'undefined') {
      ...
      _saveEstimate({ destination: destKey, program: prgEl.value,
...
/* ── Hero Canvas: 세계 네트워크 지도 (v2 — enhanced) ───────────────── */
(function () {
  ...
  function bezPt(ax, ay, mx, my, bx, by, t) {
    const u = 1 - t;
    return [u*u*ax + 2*u*t*mx + t*t*bx, u*u*ay + 2*u*t*my + t*t*by];
  }

  /* 앰비언트 먼지 파티클 */
  const AMBIENT = Array.from({ length: 75 }, () => ({
  ...
  EDGES.forEach(([a, b]) => {
    for (let k = 0; k < 2; k++) {
      particles.push({
        a, b,
        t: (k * 0.5 + Math.random() * 0.3) % 1,
        spd: 0.00045 + Math.random() * 0.00085,
      });
    }
  });
  ...
  function draw(ts) {
    ...
    particles.forEach(p => {
      ...
      const [ax, ay] = toPixel(p.a, W, H);
      const [bx, by] = toPixel(p.b, W, H);
      const [mx, my] = ctrlPt(ax, ay, bx, by);
      const headA = Math.sin(p.t * Math.PI);

      /* 트레일 */
      for (let i = TRAIL_LEN; i >= 0; i--) {
        const tt = p.t - i * 0.015;
        if (tt < 0) continue;
        const [px, py] = bezPt(ax, ay, mx, my, bx, by, tt);
        const fade = (1 - i / TRAIL_LEN) * headA;
        ctx.beginPath();
        ctx.arc(px, py, 2.0 * (1 - i / TRAIL_LEN), 0, Math.PI * 2);
        ctx.fillStyle = `rgba(215,15,35,${fade * 0.85})`;
        ctx.fill();
      }

      /* 헤드 글로우 */
      const [hx, hy] = bezPt(ax, ay, mx, my, bx, by, p.t);
      const g = ctx.createRadialGradient(hx, hy, 0, hx, hy, 10);
      g.addColorStop(0, `rgba(255,60,80,${headA * 0.6})`);
      g.addColorStop(1, 'rgba(200,0,30,0)');
      ctx.beginPath();
      ...
      ctx.fillStyle = `rgba(255,215,215,${headA})`;
      ctx.fill();
    });
    ...
    CITIES.forEach(([,, sz], i) => {
      ...
      for (let ring = 0; ring < 2; ring++) {
        const phase = ((now + i * 580 + ring * 1350) % 2700) / 2700;
        ctx.beginPath();
        ctx.arc(cx, cy, sz + phase * sz * 5.5, 0, Math.PI * 2);
        ctx.strokeStyle = `rgba(204,0,26,${0.52 * (1 - phase)})`;
        ctx.lineWidth = 1;
        ctx.stroke();
      }

      /* 글로우 헤일로 */
      const grd = ctx.createRadialGradient(cx, cy, 0, cx, cy, sz * 7);
      grd.addColorStop(0, 'rgba(204,0,26,0.30)');
      grd.addColorStop(1, 'rgba(204,0,26,0)');
      ctx.beginPath();
      ...
      ctx.fillStyle = 'rgba(255,205,210,0.96)';
      ctx.fill();
    });
  ...
  '몽골': [
    {
      ...
      days: [
        ...
        { day:4, title:'자연 · 역사', am:'아르항가이 지역 화산 지형 탐방 (선택)', pm:'징기스칸 기마상 복합단지 방문', eve:'울란바토르 복귀 · 팀 성과 공유', tip:'강수량 7-8월 집중 — 우비 준비 권장' },
...
/* ════════════════════════════════════════════════════════════════════
   ...
   ════════════════════════════════════════════════════════════════════ */
const _FALLBACK_MAP = {
  '오사카':'도쿄','후쿠오카':'도쿄','나고야':'도쿄','삿포로':'도쿄','오키나와':'도쿄',
...
function getItineraries(destKey, programType) {
  /* 1. DB 키 결정 */
  const resolvedKey = ITINERARY_DB[destKey] ? destKey
    : (_FALLBACK_MAP[destKey] && ITINERARY_DB[_FALLBACK_MAP[destKey]] ? _FALLBACK_MAP[destKey] : '싱가포르');

  const courses = ITINERARY_DB[resolvedKey] || ITINERARY_DB['싱가포르'];

  /* 2. 프로그램 유형 기반 우선순위 적용 */
  if (programType && courses.length >= 2) {
    const priority = (PROGRAM_PRIORITY[resolvedKey] || {})[programType];
    if (priority) {
...
function openEstimateWindow() {
  ...
  const programType  = programEl.value;  // 'language' | 'leadership' | 'industry' | 'academic'
  ...
  const shareData = {
    ...
    itiA: { t: itiA.title, s: itiA.subtitle, h: itiA.highlights },
    itiB: { t: itiB.title, s: itiB.subtitle, h: itiB.highlights },

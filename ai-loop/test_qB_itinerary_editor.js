...
(async () => {
  ...
  /* ⚠ 목적지를 늘리면 여기서 걸린다 — 일부러 그렇게 해 뒀다(일정을 빠뜨리면 그
     목적지 견적서에 일정이 통째로 빈다). 2026-08-11 TD·TE에서 58곳이 됐고, 2026-08-20 VC에서 60곳이 됐다. */
  ok('요율표 목적지 수만큼 일정이 있다', itiKeys.length === 60, String(itiKeys.length));
  ...
  ok('모든 목적지에 방식 A·B가 있다(전제)',
    (dataSrc.match(/^\s+a: \{ tag:/gm) || []).length === 60
    && (dataSrc.match(/^\s+b: \{ tag:/gm) || []).length === 60,

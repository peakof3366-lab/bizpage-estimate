...
(async () => {
  ...
  console.log('\n[10] 담당자가 화면에서 이 표시를 이해한다');
  {
    ...
    ok('⑫ 차수가 무엇인지 안내에 있다', /같은 문의로 견적서를 다시 내면[\s\S]{0,60}차수/.test(ADMIN));
    ok('⑫ 옛 줄로 응대하지 말라고 말한다', /최신본 번호[\s\S]{0,60}응대하지 마세요/.test(ADMIN));
    ok('⑫ 고치는 법도 말한다', /개정 아님으로[\s\S]{0,40}끊을 수 있습니다/.test(ADMIN));
  }

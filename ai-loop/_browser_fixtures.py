...
🔴 **픽스처는 v2여야 한다**(`doc` 칸이 있어야 한다). 서버는 발급할 때 표준 양식 문서를
  만들어 payload에 싣는데(2026-09-17 양식 통일), `virtual_journey`가 그걸 안 붙이던
  시절의 픽스처는 **v1**이었다. 그래서 이 검사들은 고객이 실제로 받는 **롤링 페이지
  (견적서·세부견적서·일정표)를 한 번도 잰 적이 없었다** — 화면은 옛 경로로 그려졌고,
  글자와 버튼이 멀쩡히 있으니 조용히 통과했다(결함 생성기 ③).
  → 지금은 `virtual_journey`가 서버와 **같은 함수**(`api/_lib/share_doc.js`)로 붙인다.

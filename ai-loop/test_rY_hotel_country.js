...
(async () => {
  ...
  /* 🔴 예전에는 「고객이 보는 견적서·공개 페이지에는 나가지 않습니다」가 **남아 있어야**
     했다(YQ에서 한 번 지웠다가 이 줄에 걸렸다). 2026-09-08 대표 지시로 그 문단을
     화면에서 걷었다 — 이제 이 사실을 지키는 것은 **글이 아니라 코드**다:
     호텔 목록을 부르는 API가 로그인한 담당자만 부를 수 있고, 견적 계산에는
     호텔명이 아예 들어가지 않는다. 글을 되살리는 것이 답이 아니다(대표가 다시 원하면 그때). */
  ok('안내 문단이 화면에서 걷혔다 (2026-09-08 대표 지시)',
    !/고객이 보는 견적서·공개 페이지에는 나가지 않습니다/.test(adminSrc));

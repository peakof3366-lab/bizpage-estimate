...
(async () => {
  ...
  /* PM(2026-07-29): 음력 연휴를 연도별 절대 날짜(LUNAR_PEAKS)로 옮기면서 '(근사)'를
     뗐다 — 이제 추정 구간이 아니라 실제 연휴일이다. 계수·경계 단언은 그대로 통과했다. */
  ok('설날 라벨=설 연휴', gp('2027-02-06','방콕').label === '설 연휴');
  ...
  ok('추석 라벨', gp('2027-09-15','방콕').label === '추석 연휴');

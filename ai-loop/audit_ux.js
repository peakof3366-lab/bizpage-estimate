...
const CSS로감춘것 = [
  ...
  /* 🔴 접이식 사이드바 묶음 (ZL, 2026-09-08). `.sb-group-items { display:none }`이고
     부모 `.sb-group`에 `.open`이 붙어야 보인다 — 그래서 **부모를 적는다.**
     ⚠ 이걸 안 적으면 접힌 메뉴 12개를 「보인다」로 세어, 사이드바가 늘 다 보이는
       것처럼 읽힌다. 위 FAQ와 정확히 같은 유형이다.
     ⚠ 그렇다고 「사라진 기능」은 아니다 — **한 번 누르면 나온다.** 이 표는 「지금
       눈에 보이는가」를 재는 것이고, 도달 가능한가는 다른 질문이다. */
  { sel: '.sb-group', 보일때: 'open' },             /* 관리자 사이드바 묶음 */
];

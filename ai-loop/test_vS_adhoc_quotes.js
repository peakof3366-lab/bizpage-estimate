...
console.log('\n[8] 화면 — 엔진 값을 미리 채우지 않고, 상태를 말한다');
{
  ...
  /* 🔴 **2026-09-08 대표 지시로 이 검사의 대상이 바뀌었다.**
     예전에는 「엔진을 안 쓰는 근거(+21~42%·표본 2건)가 **화면에** 적혀 있는가」를 쟀다.
     대표가 「설명글을 아주 간소화」하라고 해서 그 근거를 **화면에서 뺐다.**

     ⚠ 그렇다고 이 검사를 지우면 안 된다 — 근거가 사라지면 다음 사람이 「왜 계산을
       안 하지?」 하고 엔진을 붙인다. 그 순간 +21~42%가 고객가로 나간다.
     → **재는 자리를 옮긴다: 화면이 아니라 소스에 남아 있는가.**
       담당자가 매번 읽을 것은 아니지만, 코드를 고치는 사람은 반드시 지나가는 자리다. */
  ok('⑧ 엔진을 안 쓰는 근거가 소스에 남아 있다 (화면에서는 뺐다 — 2026-09-08)',
    /\+21\.5%·\+41\.7%|21~42%/.test(ADMIN),
    '근거를 통째로 지우면 다음 사람이 엔진을 붙인다');
  /* 🔴 **2026-09-08: 이 검사도 재는 자리를 옮겼다** (대표가 그 문구를 화면에서 뺐다).
     예전에는 「화면이 **말하는가**」를 쟀다. 그런데 지키려던 것은 문장이 아니라
     **1회용 견적이 고객 목록에 안 뜬다는 보장**이고, 그 보장은 화면이 아니라
     **서버가** 한다(`listPublicPackages`의 `kind = 'catalog'`).
     ⚠ 문구가 없어도 동작은 그대로다. 오히려 문구로 재던 것이 약한 검사였다 —
       문구를 지우면 통과하고, 서버 조건을 지우면 통과하는 자가 없었다.
     ⚠ 화면에서 안내가 사라진 것은 대표 판단이다(글이 많다). 안내가 아니라
       **보장**이 남아 있는지를 여기서 지킨다. */
  ok('⑧ 1회용은 고객 목록에 안 나간다 — **서버가 거른다**',
    /listPublicPackages[\s\S]{0,300}kind = 'catalog'/.test(LIB),
    '`listPublicPackages`에서 kind 조건이 사라지면 남의 손님 견적이 상품인 척 뜬다');
  ok('⑧ 발급이 막히면 이유를 말한다',

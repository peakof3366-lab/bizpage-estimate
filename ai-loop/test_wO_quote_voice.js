  ...
  return new Promise((resolve) => {
    ...
    const dom = new JSDOM(read('estimate-view.html'), {
      ...
      beforeParse(w) {
        ...
        /* 🔴 XK부터 엑셀/CSV를 가르는 자가 **별도 파일**(`sheet_download.js`)이다.
           이 하네스는 바깥 스크립트를 안 싣기 때문에, 안 넣어 주면 화면이
           「다운로드 기능을 불러오지 못했습니다」로 빠져 **시트가 아예 안 만들어진다**
           — 검사가 화면 결함이 아니라 **하네스 결함**으로 8건 빨갛게 뜬다(실제로 겪었다).
           ⚠ 실제 화면과 같은 파일을 그대로 읽어 넣는다(요약본을 만들면 갈라진다). */
        w.eval(read('sheet_download.js'));

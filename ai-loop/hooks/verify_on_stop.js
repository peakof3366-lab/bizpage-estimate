...
const WATCH = [
  ...
  /^manual\.html$/,
  /^api\//, /^ai-loop\/[^/]+\.js$/,
  /* 🔴 2026-09-14: `admin.html`을 화면 단위로 쪼개면서 **10,000줄이 이 목록 밖으로
     나갔다.** 그대로 두면 「요율 화면만 고친 턴」에 검증이 **통째로 안 돈다** —
     안전망이 있는 줄 알면서 없는 상태가 되고, 이 저장소가 가장 여러 번 당한 모양이다.
     ⚠ 화면을 더 뗄 때마다 여기를 고칠 필요가 없도록 **폴더째** 건다. */
  /^admin\//, /^admin\.css$/,
];

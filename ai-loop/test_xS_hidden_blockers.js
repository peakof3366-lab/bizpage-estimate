...
(async () => {
  ...
  console.log('\n[9] 🔴 상담 신청 — 담당자가 「누구 회사인지」를 알 수 있어야 한다');
  {
    ...
    if (req) {
      const b = req.body || {};
      /* 🔴 2026-09-14 대표 지시로 견적 폼에서 회사명 칸을 뺐다 — 그래서 이 상담
         신청에는 **소속이 안 실린다.** 그 사실을 여기서 잠근다.
         ⚠ 잃은 것을 적어 둔다: 견적 결과에서 바로 누르는 상담은 이제 이름·연락처만
           온다. 담당자는 전화해서 물어야 한다.
         ✅ 안 잃은 것: 하단 문의 폼(#inqOrg)은 회사/기관명을 **필수로** 받는다
           (아래 「일반 문의에도 소속이 실린다」가 그것을 잡는다). */
      ok('⑨ 견적 폼 소속이 더는 안 실린다(칸을 뺐다)', !String(b.org || '').trim(),
        '보낸 값: [' + (b.org || '') + ']');
  ...
  console.log('\n[11] 🔴 결재에 붙는 엑셀 — 견적번호와 유효기간이 없었다');
  {
    ...
    if (Array.isArray(sheet)) {
      ...
      /* 🔴 2026-09-14: 회사명·담당자 칸을 뺐으므로 이 두 줄은 **아예 안 찍힌다.**
         예전 모양대로 두면 견적서마다 「신청 기관: —」·「담당자: —」가 늘 나가
         문서가 덜 만들어진 것처럼 보인다. 그래서 `script.js`에서 **빈 줄을 넣지 않게**
         고쳤고, 여기서 그 사실을 잠근다.
         ⚠ 담당자가 관리자에서 값을 채우면 줄이 다시 나온다 — 그때는 아래가 실패하므로
           그 상황이 생기면 이 단언을 조건부로 바꿔야 한다. */
      ok('⑪ 빈 신청 기관·담당자 줄을 넣지 않는다',
        !label('신청 기관') && !label('담당자'),
        String(label('신청 기관')) + ' / ' + String(label('담당자')));

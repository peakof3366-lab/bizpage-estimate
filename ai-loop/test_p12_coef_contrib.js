...
(async () => {
  ...
  /* ⚠ 2026-09-15 대표 지시로 문구를 줄였다(「필요 없어 보이는 문구는 간결하게」).
     예전: 「이 견적은 계수 스냅샷 도입(2026-07) **이전에 생성**되어 계수 기여도 정보가
            없습니다. 이후 생성된 견적부터 표시됩니다.」 → 「2026-07 이전 견적이라 계수
            기록이 없습니다.」
     🔴 지키려는 것은 **「왜 비었는지 말한다」**이지 특정 낱말이 아니다 — 아무 말도 없이
       빈 패널을 두면 「원래 없는 기능」으로 읽힌다(조용한 폴백). 뜻으로 옮긴다. */
  ok('왜 비었는지 말한다(조용히 비우지 않는다)',
    /없습니다/.test(h2) && /2026-07/.test(h2), h2.slice(0, 120));

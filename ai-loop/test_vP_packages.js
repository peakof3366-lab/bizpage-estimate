...
console.log('\n[7] 못 불러온 것을 「0건」으로 보여주지 않는다');
{
  ...
  /* WE: 목록이 둘(상품·소규모 견적)이라 문구가 「없다」로 넓어졌다 — 같은 함수가
     두 상자에 같은 말을 쓴다. 「0건」으로 그리지 않는다는 규칙 자체는 그대로다. */
  ok('⑦ 관리자 목록이 실패를 구분해 말한다', /「없다」는 뜻이 아닙니다/.test(ADMIN));
...
console.log('\n[10] 패키지 견적서 — 값이 브라우저를 안 지난다 (VR)');
{
  ...
  /* ⚠ WS(2026-08-26)에서 맞춤 견적 쪽 문구가 **「VAT 별도」로 줄었다** — 「부대비용
     미포함」이 자기 항목표와 모순됐기 때문이다(TP부터 「현장 부대비용」이 총액에 들어간다).
     이 검사가 지키려던 것은 문구 자체가 아니라 **패키지와 맞춤이 갈려 있는가**이므로,
     새 문구를 고정하도록 고쳤다(지우지 않았다). */
  ok('⑩ 맞춤 견적 기준 문구를 패키지에 안 찍는다', /d\.pkg[\s\S]{0,260}: 'VAT 별도'/.test(EV));
  ok('⑩ 그 문구에 「부대비용 미포함」이 안 남았다',
    !/'VAT 별도 · 부대비용 미포함'/.test(EV));

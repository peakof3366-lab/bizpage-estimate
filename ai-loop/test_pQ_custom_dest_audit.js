...
console.log('[1] 정상 커스텀 목적지 — 오류가 없어야 한다 (거짓 경보 방지)');
{
  ...
  if (assertLive(r, '정상')) {
    ...
    /* ⚠ 목적지를 늘리면 여기서 걸린다 — **일부러 그렇게 해 뒀다**(빠뜨림 방지).
       2026-08-11 TD에서 가고시마·후아힌을 넣어 57개가 됐고,
       2026-08-20 VC에서 아오모리·미야코지마를 넣어 60개가 됐다. */
    ok('정적 기준 개수가 요율표와 같다', /정적 요율표 기준 60개/.test(r.out));
  }
}

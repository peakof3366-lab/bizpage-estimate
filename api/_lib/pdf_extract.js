...
/* 🔴 **`\bTC\b`의 `\b` 두 개가 실제 백스페이스 문자(0x08)로 들어가 있었다** (ZB).
   그런 문자열은 PDF 본문에 있을 수 없으니, 「TC」(인솔자) 식사 줄을 스태프로
   걸러내는 규칙이 **한 번도 작동한 적이 없다.** 에러가 아니라 「안 걸림」이라 조용하다.
   ⚠ 원인은 파일을 `\b`가 이스케이프되는 경로로 다시 쓴 것이다(파이썬 비-raw 문자열
     등). 이 저장소에 같은 흔적이 셋 있었다 — 코드를 도구로 다시 쓸 때 주의할 것. */
const MEAL_STAFF_RE = /가이드|기사|인솔|스텝|스태프|\bTC\b/i;

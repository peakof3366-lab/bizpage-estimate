...
/* ═══ 현장 부대비용 (TP) — **요율 9칸에 담기지 않는 돈** ═══════════════════════
   왜 생겼나 — 46건 코퍼스를 항목 단위로 전부 매핑해 보니(audit_item_taxonomy),
   견적서 돈의 **12.4%가 우리 아홉 칸 어디에도 안 들어간다.** 실제 줄을 열어 보면:

     기관 섭외비   「섭외비 700,000 x 3회」          (회의록 2번이 지목한 항목)
     통역비        「기관섭외 통역비 362,500 x 3회」
     국내수송      「솔라티(광주-인천 왕복) 1,400,000」
     싱글차지      「SGL SUPP 505,600 x 3명」
     행사운영      현수막·기념품·네임텐트·사진·MC

   그리고 오차 쪽에서 같은 숫자가 나왔다(audit_gap_source):
     고객용 견적서 12건 기준 계통 편향 **-12.1%** (사분위 폭은 8.5%로 매우 좁다)
   **두 숫자가 거의 같다.** 즉 엔진이 12% 싼 이유는 목적지 단가가 낮아서가 아니라
   **이 항목들을 담을 칸이 없어서**다.

   ⚠ 그래서 **목적지 단가를 올리지 않는다.** 실제로 해 봤다 — 식비 x2.5 · 유류 x0.5
     일괄 보정은 중앙값을 -9.1%에서 -5.0%로 당기면서 **사분위 폭을 24.4%->25.6%로
     벌렸다**(TN에서 기각). 틀린 칸을 올려 우연히 총액을 맞추는 상태였다.

   ⚠ **기준은 지상비다.** 부대비용은 현장 진행에 비례하지 항공권 값에 비례하지 않는다
     (섭외·통역·수송·행사운영은 전부 현지에서 쓰는 돈이다). 항공까지 포함한 총액에
     비율을 걸면 장거리 노선에서 부대비용이 근거 없이 부풀어 오른다.

   ⚠ **계수 값은 추정이 아니라 실측으로 고른다.** `ai-loop/sim_ancillary.js`가 0~20%를
     쓸어 보고 [중앙값 + 사분위 폭]이 가장 좋은 값을 낸다. 값을 바꿀 때는 그 자를 다시
     돌릴 것 — 중앙값만 보면 폭이 벌어지는 것을 놓친다. */
const ANCILLARY = {
  /* 지상비(호텔+식사+차량+가이드+관광)에 대한 비율.
     ⚠ **전혀 다른 두 측정이 같은 값을 가리켰다.** 그래서 이 20%는 12건에 맞춘 숫자가 아니다:
       ① `sim_ancillary.js` 스윕 — 고객용 견적서 12건에서 |오차| 중앙 12.1% → **6.3%**,
          ±10% 안 5건 → **9건**. 0~25%를 쓸어 20%가 가장 좋았다.
       ② `audit_item_taxonomy.js` — 우리 아홉 칸 밖으로 새는 돈이 **총계의 12.4%**이고,
          지상비가 총계의 60.1%다. 12.4 ÷ 60.1 = **20.6%**.
     ①은 오차에서, ②는 견적서 줄에서 각각 얻은 값인데 20% 언저리에서 만난다.
     한쪽만 있었으면 12건에 과적합한 숫자였을 것이다.

     ⚠ **스윕이 가리키는 값은 흔들린다 — 따라가지 말 것.** 운영 오버라이드를 얹고 다시
       쓸어 보니 25%가 뽑혔는데(±10% 8건), 20%는 ±10% 7건이면서 **±5%는 4건으로 더 많고
       |오차| 중앙값도 7.0%로 25%(8.3%)보다 낫다.** 표본이 12건뿐이라 이 정도 차이는
       잡음이다. 그래서 **스윕의 1등이 아니라 ②의 독립 근거가 있는 20%를 유지한다.**
       스윕은 「대략 어느 자리인가」를 말해 줄 뿐이고, 자리를 고정하는 것은 ②다. */
  rate: 0.20,
  label: '현장 부대비용',
  /* 고객 화면에 그대로 나가는 설명 — 무슨 돈인지 안 밝히면 "왜 붙었냐"가 된다 */
  note: '기관 섭외·통역, 행사 운영(현수막·기념품·사진), 현장 진행비 등',
};

/* B안 — **PM이 아는 것을 넣으면 더 정확해진다.** 고객은 안 골라도 되고(A안 계수가
   기본으로 깔린다), 담당자가 아는 조건을 켜면 그만큼 얹는다.
   ⚠ 값은 전부 **견적서에서 실제로 뽑은 것**이다. 표본이 얇은 것은 그 사실을 적는다 —
     모르는 숫자를 그럴듯하게 채우면 그게 고객이 보는 금액이 된다. */
const ANCILLARY_OPTIONS = [
  {
    key: 'agency', label: '기관 방문·섭외',
    /* 실측: 「섭외비 700,000 x 3회」 + 「기관섭외 통역비 362,500 x 3회」 (신한 이태리 100명)
       = 1회당 약 1,062,500. 방문 1회를 기본으로 본다. */
    perVisit: 1062500, unit: '회',
    note: '기관 섭외비 + 통역비 (실측 1건 — 견적서가 쌓이면 갱신 제안이 알려 준다)',
  },
  {
    key: 'domestic', label: '지방 출발(국내 수송)',
    /* 실측: 「솔라티(광주-인천 왕복) 1,400,000」 (호남대 북해도) — 15인승 1대 왕복. */
    perVehicle: 1400000, capacity: 15, unit: '대',
    note: '출발지→공항 왕복 전세 (실측 1건, 15인승 기준)',
  },
];
...
/* ═══ 금액 구간별 마진 계수 (VK) — **저가 구간이 원가에 못 미치던 것을 메운다** ═══
   ...
   ⚠ **배수는 마진 두 줄에 함께 걸린다** — 💼 본사 수익과 🏷️ 현지 수익금이 같은 값에서
...
const DEST_CLASSIFY = {
  ...
  '아오모리':   { zone:'short', ins:'asiaShort', region:'일본',        country:'일본',      currency:'JPY', season:'japan'         },
  '오키나와':   { zone:'short', ins:'asiaShort', region:'일본',        country:'일본',      currency:'JPY', season:'japan'         },
  /* VC: 오키나와 본섬과 **다른 섬**이다. 2026-08-13에 오키나와로 묶었다가
     「오키나와가 원가보다 30.4% 싸다」가 나왔고, 원인이 다른 섬의 원가를 오키나와
     요율로 잰 것이었다. 대표 방침(2026-08-20): **지역이 다르면 별도로 추가한다.** */
  '미야코지마':  { zone:'short', ins:'asiaShort', region:'일본',        country:'일본',      currency:'JPY', season:'japan'         },
...
/* =====================================================================
   ...
   탓에 같은 권역 안에서도 예외가 있습니다(예: 삿포로 겨울, 오키나와 여름).
   ⚠ **하와이 겨울은 2026-09-17에 해결**했습니다 — 전용 프로파일 `hawaii`로 뗐습니다.
     삿포로·오키나와는 **일부러 안 쪼갠 것**입니다(`japan` 프로파일 주석: 쪼갤 근거가
     없어 9곳이 한 표를 쓴다. 실측이 쌓이면 그때 나눈다 — 추정으로 나누지 않는다).
...
const DEST_SEASON_PROFILES = [
  ...
  {
    /* 일본 — 2026-09-15 대표 지시로 **온라인 조사 후 다시 짰다.**
       ■ 고치기 전: 성수기[3,4,7,8,10,11] · 비수기[1,6]
       ■ 조사(한국 출발 항공권): 성수기 1~2월(삿포로 눈축제 60~80만)·4~5월(골든위크
         50~60만, 평소 20만대)·8월(오본)·12월말~1월초(연말연시 40~50만) /
         **비수기 3월 중순·6월(장마, 연중 최저)·11월(10만원 후반대)**.
         벚꽃철 호텔은 평소보다 **20~30% 상승**(도쿄 비즈니스호텔은 전년 대비 2배).

       ■ 🔴 그런데 **`PEAK_CALENDAR`가 짧은 피크를 이미 날짜로 가산한다**:
             벚꽃 3/25~4/10 ×1.20 · 일본 골든위크 4/27~5/06 ×1.35 ·
             연말연시 12/20~1/03 ×1.25 · 여름 7/15~8/20 ×1.20
         월 시즌표가 3·4월을 또 성수기로 잡으면 항공·유류에 **1.15 × 1.20 = 1.38배**가
         걸린다 — **이중 반영**이다. 이 파일 머리말이 「월 단위 넓은 시즌 / 날짜 단위
         짧은 피크」로 역할을 갈라 둔 이유가 그것이다.
       → 월 시즌표는 **넓은 시즌만** 맡고, 벚꽃·골든위크·연말은 PEAK_CALENDAR에 맡긴다.

       ■ 그래서 이렇게 정했다
         · **7·8월 성수기 유지** — 방학 수요는 월 전체에 걸린다.
         · **3·4·10월은 평시로** — 벚꽃·골든위크는 날짜로 이미 잡히고, 그 밖의 날은
           조사상 비수기다(3월 중순 도쿄 20만대 초반).
         · **11월은 비수기로** — 항공 10만원 후반대. 단풍철 호텔 상승 근거는 못 찾았다.
         · **1월을 비수기에서 뺀다** — 연말연시(1/1~3)와 눈축제가 있는 달을 −10%로
           매기는 것은 명백히 틀렸다. 다만 1월 전체가 성수기라는 근거도 없어 **평시**로.
       ⚠ 삿포로(눈축제)·오키나와(아열대)는 성격이 다르지만 **쪼갤 근거가 없어** 9곳이
         한 표를 그대로 쓴다. 그 목적지 실측이 쌓이면 그때 나눈다(추정으로 나누지 않는다).
       ⚠ 계수(+15% / −10%)는 그대로 — 고친 것은 **달**이다. */
    id: 'japan', name: '일본 (여름 7~8월 성수기 / 6·11월 비수기 · 벚꽃·골든위크는 날짜로 가산)',
    config: [
      { id:'peak',    months:[7,8],   factor:1.15, label:'여름 성수기', badge:'여름 성수기 +15%' },
      { id:'offpeak', months:[6,11],  factor:0.90, label:'비수기',     badge:'비수기 −10%' },
      { id:'normal',  months:[],      factor:1.00, label:'평시',       badge:'평시' },
    ],
  },
  {
...
const destinationRates = [
  ...
  {"destination_key":"아오모리",     "label":"아오모리",     "airfare":380000, "fuel_surcharge":180000,"hotel_per_room":250000,"meal_per_person":25000, "vehicle_large":1150000,"vehicle_small":800000, "guide_fee":300000,"sightseeing_fee":30000, "margin_per_traveler":130000, "rateDate":"2026-08", "notes":"단가·시즌 안내는 같은 일본 북부인 삿포로를 복사한 출발점입니다 — 견적서가 쌓이면 갱신 제안이 알려줍니다. 실측 1건(굿리치 마케팅임원 워크샵 23명 3일)은 항공 518,000·유류 145,000·호텔 392,000으로 이 출발점보다 높습니다 — 1건이라 아직 요율로 굳히지 않았습니다.", "season_note":"성수기: 7~8월(선선한 여름)·12~2월(눈축제·스키) · 평시: 9~10월(단풍) · 비수기: 3~4월(잔설)·5월"},
  ...
  {"destination_key":"미야코지마",    "label":"미야코지마",    "airfare":360000, "fuel_surcharge":180000,"hotel_per_room":250000,"meal_per_person":25000, "vehicle_large":1100000,"vehicle_small":770000, "guide_fee":300000,"sightseeing_fee":30000, "margin_per_traveler":130000, "rateDate":"2026-08", "notes":"⚠ 오키나와 본섬과 다른 섬입니다 — 2026-08-13에 오키나와로 묶었다가 「오키나와가 원가보다 30.4% 싸다」가 나와 떼어냈습니다. 단가·시즌 안내는 오키나와를 복사한 출발점일 뿐입니다. 실측 1건(굿리치 재무분석 워크샵 19명 4일): 항공 320,000·유류 90,000·호텔 175,750·차량 1,140,000·가이드 285,000·관광 30,450 — 1건이라 아직 요율로 굳히지 않았습니다. 골프 요금은 실측이 없어 비워 뒀습니다(화면이 옵션을 잠급니다).", "season_note":"성수기: 7~9월(해변) · 평시: 4~6월·10월 · 비수기: 12~2월. 태풍 8~9월 주의"},
  /* ── 동북아시아 : 홍콩 · 마카오 ── */
...
if (typeof module !== 'undefined' && module.exports) {
  ...
  module.exports.ANCILLARY_OPTIONS = ANCILLARY_OPTIONS;
}
...
const DEST_REC = {
  ...
  '아오모리': {
    a: { tag:'역량강화형', desc:'혼슈 최북단의 농림수산 6차산업과 사과 클러스터 학습',
         points:['히로사키 사과 가공·유통 클러스터 견학','아오모리항 수산물 경매·콜드체인 시찰','지역 6차산업화 사례 세미나'],
         items:['사과 재배·가공·수출 일관 체계 견학','아오모리항 수산 유통 현장 투어','지역 브랜드화 사례 강의','현지 제조·식품 기업 교류'],
         value:'인구 감소 지역이 1차 산업을 브랜드로 바꿔 낸 과정을 현장에서 학습' },
    b: { tag:'동기부여·화합형', desc:'네부타와 설경, 혼슈 최북단에서의 몰입형 팀 연수',
         points:['네부타 축제 박물관·제작 체험','핫코다 설경 트레킹 또는 스키','오이라세 계류 산책'],
         items:['네부타 등롱 제작 조별 체험','핫코다 설경·오이라세 계류 팀 활동','아오모리 수산시장 아침 미션','향토식 팀 만찬'],
         value:'축제 제작을 조별로 완성하며 협업을 체감하는 몰입형 팀빌딩' },
  },
  '오키나와': {
  ...
  '미야코지마': {
    a: { tag:'역량강화형', desc:'섬 관광·리조트 운영과 재생에너지·물 자립 인프라 학습',
         points:['리조트 운영·인력 수급 사례 세미나','섬 상수·해수담수화 인프라 견학','지역 관광 마케팅 좌담회'],
         items:['리조트 운영 구조 브리핑','해수담수화·상수 인프라 시찰','섬 관광 마케팅 사례 강의','지역 사업자 교류 세션'],
         value:'본섬과 조건이 다른 낙도에서 관광 산업이 자립을 만드는 방식을 현장에서 학습' },
    b: { tag:'동기부여·화합형', desc:'미야코블루 바다에서의 해양 액티비티 팀 연수',
         points:['이라부 대교 드라이브·전망 포인트','스노클링·카약 팀 활동','요나하 마에하마 해변 팀 미션'],
         items:['이라부 대교 팀 드라이브','스노클링·카약 조별 활동','해변 팀 미션·단체 촬영','섬 향토식 BBQ 만찬'],
         value:'이동 거리가 짧아 일정 밀도가 높은 섬 조건을 살린 고몰입 팀 활동' },
  },
...
const ITINERARY_DB = {
  ...
  /* ─── 아오모리 ──────────────────────────────────────────────────── */
  /* VC: 실측 견적서는 **3일**(굿리치 마케팅임원 23명)인데 여기 초안은 4일로 둔다 —
     이 창고는 「고객에게 그대로 나가는 정답」이 아니라 작성자가 꺼내 쓰는 출발점이고,
     일정은 견적서마다 작성자가 확정한다(UJ 이후 방침). */
  '아오모리': [
    {
      title: '아오모리 1차산업 6차산업화 벤치마킹 코스',
      subtitle: '사과·수산 클러스터가 지역 브랜드로 바뀐 과정을 현장에서 학습',
      highlights: ['히로사키 사과 가공·유통 클러스터','아오모리항 수산 유통 현장','지역 브랜드화 사례 세미나','현지 식품기업 교류'],
      days: [
        { day:1, title:'입국 · 오리엔테이션', am:'아오모리공항 도착, 호텔 체크인', pm:'시내 오리엔테이션 미팅', eve:'향토식 환영 만찬', tip:'공항–시내 약 35분 — 단체 버스 사전 예약 권장' },
        { day:2, title:'사과 클러스터', am:'히로사키 사과 재배·가공 현장 견학', pm:'가공·수출 유통 체계 브리핑', eve:'히로사키 팀 저녁', tip:'수확기(9~11월)는 현장 방문 예약이 조기 마감된다' },
        { day:3, title:'수산 유통 · 브랜드화', am:'아오모리항 수산물 시장·콜드체인 시찰', pm:'지역 브랜드화 사례 세미나 · 현지 기업 교류', eve:'팀 만찬', tip:'경매 견학은 이른 아침이라 전날 일정 강도를 낮춘다' },
        { day:4, title:'정리 · 귀국', am:'연수 성과 공유 세션', pm:'특산품 쇼핑 · 공항 이동', eve:'귀국', tip:'겨울철은 강설로 공항 지연이 잦다 — 여유 시간 확보' },
      ],
    },
  ],
  ...
  /* ─── 미야코지마 ────────────────────────────────────────────────── */
  /* ⚠ 오키나와 본섬 코스를 그대로 쓰면 안 된다 — 츄라우미 수족관·나키진성은 본섬이고
     여기서 갈 수 없다. 섬을 나누기로 한 이상 일정도 나눈다(대표 방침 2026-08-20). */
  '미야코지마': [
    {
      title: '미야코지마 섬 관광산업·인프라 연수 코스',
      subtitle: '본섬과 조건이 다른 낙도에서 관광이 자립을 만드는 방식을 학습',
      highlights: ['리조트 운영·인력 수급 사례','해수담수화·상수 인프라 견학','이라부 대교 인프라 시찰','지역 사업자 교류'],
      days: [
        { day:1, title:'입국 · 오리엔테이션', am:'미야코공항 도착, 리조트 체크인', pm:'섬 개황 브리핑 · 오리엔테이션', eve:'향토식 환영 만찬', tip:'공항–리조트 20~30분 — 섬 전체가 가까워 이동 손실이 적다' },
        { day:2, title:'관광산업 현장', am:'리조트 운영 구조 브리핑 · 객실/인력 수급 사례', pm:'지역 관광 마케팅 좌담회', eve:'팀 저녁', tip:'성수기(7~9월)는 리조트 회의실 확보가 어렵다 — 3개월 전 선점' },
        { day:3, title:'섬 인프라', am:'해수담수화·상수 인프라 시찰', pm:'이라부 대교 이동 · 도서 연결 인프라 견학', eve:'해변 BBQ 팀 만찬', tip:'대교 구간은 전망 정차 지점이 한정적 — 코스 사전 확정 권장' },
        { day:4, title:'정리 · 귀국', am:'성과 공유 세션', pm:'공항 이동', eve:'귀국', tip:'8~9월 태풍 시즌은 결항 대비 일정 여유가 필요하다' },
      ],
    },
  ],
...
const PROGRAM_TYPES = {
  ...
  /* 🔴 휴양 (VV) — 앞 넷과 **성격이 다른 하나**다. 연수가 아니라 일반 고객의 여행이고,
     인원이 적다(가족·소수 모임). 대표 지시(2026-08-24): 「일반고객 대상으로 휴양 관련
     부분을 새로 추가하고 싶다.」
     ⚠ 이 유형을 고르면 **차량·가이드가 기본으로 꺼진다**(script.js). 실측이 정한 것이다 —
       그 둘이 인원과 무관한 **정액**이라 소수인원에서 1인당으로 나누면 폭발한다:
         오키나와 3박4일 4명 · 차량 3,080,000 + 가이드 836,000 → 1인당 979,000원
         전부 포함 2,320,246원(소매가 대비 +95.0%) → 차량·가이드 빼면 1,208,446원(**+1.6%**)
       가족여행은 38인승 버스도 전담 가이드도 쓰지 않는다. 켜 둔 채로 열면 계산기가
       일반 고객에게 **소매가의 두 배**를 부른다. */
  leisure:    { label: '휴양 / 자유여행',      short: '휴양' },
};
